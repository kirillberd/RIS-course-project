    ...
    def _make_query_conditions(self, query_obj: BillboardQuery) -> dict:
        ...
        conditions['schedule_clause'] = f"""
            AND b.id NOT IN (
                ...
                FROM schedule 
                WHERE (
                    (date_begin BETWEEN '{query_obj.date_start}' AND '{query_obj.date_end}')
                    OR (date_end BETWEEN '{query_obj.date_start}' AND '{query_obj.date_end}')
                    OR ('{query_obj.date_start}' BETWEEN date_begin AND date_end)
                    OR ('{query_obj.date_end}' BETWEEN date_begin AND date_end)
                )
            )
        """
        ...
        if query_obj.direction:
            conditions['direction_clause'] = f"AND b.direction LIKE '%{query_obj.direction}%'"
        ...
        if query_obj.min_quality:
            conditions['quality_clause'] = f"AND b.quality_indicator >= {query_obj.min_quality}"
